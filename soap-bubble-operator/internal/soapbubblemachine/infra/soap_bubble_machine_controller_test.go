...
var _ = Describe("SoapBubbleMachine Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind SoapBubbleMachine")
			err := k8sClient.Get(ctx, typeNamespacedName, soapbubblemachine)
			if err != nil && errors.IsNotFound(err) {
				resource := &soapbubbleoperatorv1alpha1.SoapBubbleMachine{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance SoapBubbleMachine")
